...
function getPool(score: number): Record<QuestionType, number> {
  ...
  return applyDelta(
    bracket.pool as Record<string, number>,
    bracket.poolDelta,
    above,
...
function getModifier(score: number): Record<ModifierType, number> {
  const { bracket, above } = getBracket(score)
  const raw = applyDelta(
    bracket.modifier as Record<string, number>,
    bracket.modifierDelta,
    above,
  )
...
function calculateQuestionRating(
  type: QuestionType,
  maxStr: string,
  modifier: ModifierType,
  age: number,
...
export function generateQuestion(user: User, forcedType: QuestionType): QuestionResult {
  ...
  const q = buildQuestion(
    forcedType,
    weightedRandom(maxDist),
    weightedRandom(modDist) as ModifierType,
    user.age,
  )
  return { ...q, meta }
}
...
export function updateUserAfterAnswer(
  ...
  const rawRating = correct
    ? user.rating + ratingGain
    : user.rating - ratingLoss

  // Level nunca diminui — apenas acertos o avançam.
  const baseLevelGain = correct
    ? Math.max(0.01, 3 * (1 + (diff / 100) * 0.02))
    : 0
  ...
  const { bonusPoints, bonusXP } = applyStreakBonus(
    basePointsGained,
    baseLevelGain,
    user.offensive ?? 0
  )
